...
<head>
   <meta charset="utf-8" />
   <title>PPMS Transporter Home</title>
   ...
   <div id="container" class="row-fluid">
      ...
      <div id="main-content">
         ...
         <div class="container-fluid">
            ...
            <div class="row-fluid">
                <!--BEGIN METRO STATES-->
                <div class="metro-nav" style="align-content: center;">
                    <div class="metro-nav-block nav-block-orange double">
                        <a data-original-title="" href="driverslist">
                            <i class="icon-user"></i>
                            <div class="info"></div>
                            <div class="status">My Drivers</div>
                        </a>
                    </div>

                    <div class="metro-nav-block nav-block-green double">
                        <a data-original-title="" href="createinvoice">
                            <i class="icon-eye-open"></i>
                            <div class="info"></div>
                            <div class="status">My Invoices</div>
                        </a>
                    </div>
                     <div class="metro-nav-block nav-block-red double ">
                        <a data-original-title="" href="#">
                            <i class="icon-eye-open"></i>
                            <div class="info"></div>
                            <div class="status">Feedbacks</div>
                        </a>
                    </div>
                 ...
                 <div class="span12">
                     <div class="widget green">
                            <div class="widget-title">
                                <h4><i class="icon-reorder"></i> My Deliveries for FORTE</h4>
									<span class="tools">
                  ...
                  for(var t in tr){
                      for( var a=0;a<tr[t].length;a++){
                          ...
                          if(transobj.cust_name === transname){
                           var transporterid = transobj.cust_id;
                           $.post('transporterhome',{'refresh':'all' , 'transporter':transporterid },function(msg,stat,xhr){
                            if(xhr.readyState === 4){
                                if(xhr.status === 200){
                                    $('#preloader').hide();
                                    $('#accept').show();
                                    setTimeout(function(){
                                        window.location.href = 'transporterhome';
                                    },2000);
                                }
                            }
                        });
                          ...
                          if(transobj.cust_name === transname){
                           ...
                           'trans_id':transid} , function(msg,stat,xhr){
                                if(xhr.readyState === 4){
                                    if(xhr.status === 200){
                                        ...
                                        window.localStorage.setItem('invoices',$.trim(msg));
                           ...
                           $.getJSON('json/'+transid+'_results.json',function(key,value){
                            //delivery number, pod date, pod time, status
                              var deliveries = key;
                              window.localStorage.setItem('mydeliveries',deliveries);
                                      ...
                                      var time = delvobj.pod_time.toString();
                                       if( delvobj.vbeln == window.delivery ){
                                           $.getJSON('json/results.json',function(k,v){
                                               var customer = JSON.parse($.trim(k));
                                               for(var c in customer){
                                                   for(var d=0;d<customer[c].length;d++){
                                                       var aCust = customer[c][d];
                                                       if(aCust.cust_id === delvobj.customer){
//                                                           alert(aCust.cust_id+'=='+delvobj.customer);
                                                           custname = aCust.cust_name;
                                                           $('#name').html(custname);
                                                           $('#address').html(custname);
                                                       }
                                                   }
                                               }
                                           });
